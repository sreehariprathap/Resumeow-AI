...
export function JDMatcherPage() {
  ...
  const handleAnalyze = async () => {
    ...
    if (!currentUser) {
      toast.error('Please log in to analyze a job description.');
      return;
    }
    ...
    try {
      ...
      if (!profileData) {
        toast.error('No resume profile found. Please complete the onboarding first.');
...
Analyze the match and respond with ONLY valid JSON in this exact structure:
{
  ...
  "coverLetterSnippet": "2-3 sentence opening tailored to this specific role and company"
      ...
      const updated = [parsed, ...history].slice(0, 5);
      await saveUserData(currentUser.uid, 'jdMatches', { matches: updated });
  ...
  const handleGenerateTailored = async () => {
    ...
    try {
      ...
      // Substitute improved bullets into profile
      const improvedProfile = { ...profile };
      improvedProfile.experiences = profile.experiences.map((exp) => ({
        ...exp,
      ...
      const latex = await generateLatexResume(improvedProfile, makeAICall);
  ...
  const copySnippet = () => {
    if (!result) return;
    navigator.clipboard.writeText(result.coverLetterSnippet).then(() => {
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {result.matchedKeywords.slice(0, 5).map((kw) => (
                        <Badge key={kw} variant="secondary" className="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 text-xs">
                          {kw}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                      {result.matchedKeywords.map((kw) => (
                        <Badge key={kw} className="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 text-xs font-normal">
                          {kw}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                      {result.missingKeywords.map((kw) => (
                        <Badge
                          key={kw}
                          title="Consider adding this to your resume"
                          className="bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 text-xs font-normal cursor-help"
                        >
                          {kw}
                        </Badge>
                      ))}
                    ...
                    {result.tailoredBullets.map((tb, i) => (
                      <div key={i} className="grid grid-cols-1 sm:grid-cols-2 gap-2 items-start">
                        ...
                        <div className="rounded-lg bg-primary/10 border border-primary/20 p-3">
                          ...
                          <p className="text-xs text-foreground">{tb.improved}</p>
                        </div>
                      </div>
                    ))}
                  </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="relative rounded-lg bg-muted/50 p-4">
                    <p className="text-sm italic text-foreground pr-8">{result.coverLetterSnippet}</p>
                    <button
                      onClick={copySnippet}
